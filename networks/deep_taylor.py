...
class DTD(nn.Module):
    ...
    def forward(self, module_stack, y, class_num, model_archi, gpu_id, R = None, layer_ind = 0):
        if R == None:
            R = torch.eye(class_num)[torch.max(y, 1)[1]]
        if gpu_id:
            R = R.to(gpu_id)
        for i in range(len(module_stack) - layer_ind):
